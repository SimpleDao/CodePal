...
| 接口 | 方法 | Endpoint |
|------|------|----------|
| 提交任务 | POST | `https://app-btebw5qkzr41-api-ra5EZDjVKkXa-gateway.appmiaoda.com/image-generation/submit` |
| 查询状态 | POST/GET | `https://app-btebw5qkzr41-api-VaOwP2jDmAga-gateway.appmiaoda.com/image-generation/task` |
...
async function submitImageGeneration(prompt: string): Promise<string> {
  const response = await fetch(
    "https://app-btebw5qkzr41-api-ra5EZDjVKkXa-gateway.appmiaoda.com/image-generation/submit",
    {
      method: "POST",
      headers: {
...
async function queryTaskStatus(taskId: string): Promise<{
  ...
  const response = await fetch(
    "https://app-btebw5qkzr41-api-VaOwP2jDmAga-gateway.appmiaoda.com/image-generation/task",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
