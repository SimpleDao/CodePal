...
async function submitImageToVideo(params: {
  ...
  const response = await fetch(
    "https://app-btebw5qkzr41-api-VaOw5VAJdQBa-gateway.appmiaoda.com/v1/video_generation",
    {
      method: "POST",
      headers: {
...
async function pollVideoStatus(taskId: string): Promise<{
  ...
  const response = await fetch(
    `https://app-btebw5qkzr41-api-eLMlPNkelVj9-gateway.appmiaoda.com/v1/query/video_generation?task_id=${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "X-Gateway-Authorization": `Bearer ${apiKey}`,
...
async function retrieveVideoFile(fileId: string): Promise<string> {
  const response = await fetch(
    `https://app-btebw5qkzr41-api-rLyOyznAK2Ba-gateway.appmiaoda.com/v1/files/retrieve?file_id=${fileId}`,
    {
      method: "GET",
      headers: {
        "X-Gateway-Authorization": `Bearer ${apiKey}`,
...
async function generateFirstLastFrameVideo(params: {
  ...
  const submitResponse = await fetch(
    "https://app-btebw5qkzr41-api-nYWNRQr5pV1L-gateway.appmiaoda.com/v1/video_generation",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  while (Date.now() < deadline) {
    ...
    if (queryJson.status === "Success") {
      ...
      const retrieveResponse = await fetch(
        `https://app-btebw5qkzr41-api-rLyOyznAK2Ba-gateway.appmiaoda.com/v1/files/retrieve?file_id=${queryJson.file_id}`,
        {
          method: "GET",
          headers: { "X-Gateway-Authorization": `Bearer ${apiKey}` },
        }
