 ...
 */
public class LoongCInlineManager {

    private static final Logger LOG = Logger.getInstance(LoongCInlineManager.class);
    ...
    // ── 单例 ───────────────────────────────────────────────
    private static final LoongCInlineManager INSTANCE = new LoongCInlineManager();
    public static LoongCInlineManager getInstance() { return INSTANCE; }
    private LoongCInlineManager() {}
    ...
    // ── 幽灵文本状态（EDT 访问） ───────────────────────────
    private Inlay<LoongCInlayRenderer> activeInlineInlay;
    private Inlay<LoongCBlockRenderer> activeBlockInlay;
    /** 当前已积累的补全文本（流式追加） */
    ...
    private void triggerFIMCompletion(@NotNull Editor editor, int caretOffset,
        ...
        try {
            ...
            System.out.println("[LoongC] triggerFIMCompletion 开始，ver=" + thisRequestVersion
                    + " offset=" + caretOffset);
            ...
            activeCall = client.streamFIM(before, after, new DeepSeekClient.FIMCallback() {
                ...
                public void onToken(String token) {
                    System.out.println("[LoongC] onToken raw=[" + token.replace("\r","↵r").replace("\n","↵n") + "]");
                    ApplicationManager.getApplication().invokeLater(
                ...
                public void onComplete() {
                    System.out.println("[LoongC] FIM 补全完成，共 " + completionBuffer.length() + " 字符");
                }

                @Override
                public void onError(Throwable t) {
                    System.out.println("[LoongC] FIM 补全出错: " + (t != null ? t.getMessage() : "unknown"));
                }
            });
        } catch (Exception e) {
            System.out.println("[LoongC] triggerFIMCompletion 异常: " + e.getMessage());
    ...
    private void appendToken(@NotNull Editor editor, int offset, @NotNull String token,
        ...
        if (requestVersion != activeRequestVersion) {
            System.out.println("[LoongC] appendToken 丢弃过期token, reqVer=" + requestVersion
        ...
        if (activeEditor != null && activeEditor != editor) {
            System.out.println("[LoongC] appendToken editor 不匹配，丢弃 token=[" + token + "]");
            return;
        }
        ...
        if (currentCaretOffset != offset) {
            System.out.println("[LoongC] appendToken 光标已移动: 期望=" + offset
        ...
        Document doc = editor.getDocument();
        if (offset > doc.getTextLength()) {
            System.out.println("[LoongC] appendToken offset 越界: offset=" + offset
        ...
        if (!currentPrefix.equals(prefixSnapshot)) {
            System.out.println("[LoongC] appendToken prefixSnapshot 不匹配，dismissCompletion");
            System.out.println("[LoongC]   期望prefix=[" + prefixSnapshot.replace("\n", "↵").replace("\r", "↵") + "]");
            System.out.println("[LoongC]   当前prefix=[" + currentPrefix.replace("\n", "↵").replace("\r", "↵") + "]");
            dismissCompletion();
            return;
        }
        ...
        System.out.println("[LoongC] appendToken sanitizedToken=[" + sanitizedToken + "]");
        ...
        if (full.trim().isEmpty()) {
            System.out.println("[LoongC] appendToken buffer 全空白，等待实质性字符，当前buffer=[" + full.replace("\n","↵") + "]");
            return;
        }
        ...
        if (stripped > 0) {
            System.out.println("[LoongC] appendToken 剥离了 " + stripped + " 个开头 \\n");
        }
        ...
        System.out.println("[LoongC] appendToken 渲染: firstLine=[" + firstLine + "] restLines=" + restLines.length);
